 ...
 * Author: 3urobeat
 *
 * Last Modified: 25.09.2023 18:41:05
                ...
                bot.community.followUser(id, (error) => {
                    ...
                    if (error) {
                        ...
                        commandHandler.data.ratingHistoryDB.insert({ id: id, accountName: activeReqEntry.accounts[i], type: "follow", time: Date.now() }, (err) => {
                            if (err) logger("warn", `Failed to insert 'follow' entry for '${activeReqEntry.accounts[i]}' on '${id}' into ratingHistory database! Error: ` + err);
                        });
