 ...
 * Author: 3urobeat
 *
 * Last Modified: 22.04.2023 18:02:26
...
module.exports.groupComment = {
    ...
    run: async (commandHandler, args, steamID64, respondModule, context, resInfo) => {
        ...
        let requesterSteamID = steamID64;
        let ownercheck       = commandHandler.data.cachefile.ownerid.includes(requesterSteamID);
        let lastCommentDoc   = commandHandler.data.lastCommentDB.findOneAsync({ id: steamID64 });
        ...
        if (config.maxComments == 0 && !ownercheck) return respond(commandHandler.data.lang.commentcmdowneronly);


        /* --------- Calculate maxRequestAmount and get arguments from comment request --------- */
        let { maxRequestAmount, numberOfComments, profileID, quotesArr } = await require("../helpers/getCommentArgs.js").getCommentArgs(args, steamID, requesterSteamID, SteamID.Type.CLAN, lang, respond);
        let receiverSteamID = profileID;

        if (!maxRequestAmount && !numberOfComments && !quotesArr) return; // Looks like the helper aborted the request (don't check for receiverSteamID as it has a default value set above)
        if (receiverSteamID == requesterSteamID) return respond(400, "You need to provide a group link or groupID!"); // Send custom error message if user forgot to provide group id


        /* --------- Check for cooldowns and calculate the amount of accounts needed for this request ---------  */
        let { allAccounts, accountsNeeded } = require("../helpers/checkAvailability.js").checkAvailability(receiverSteamID, numberOfComments, true, lang, res, lastcommentdoc, respond);
        ...
        let accountOrder = require("../helpers/getAccountOrder.js").getAccountOrder(false, allAccounts, accountsNeeded, numberOfComments, requesterSteamID, receiverSteamID, lang, respond);
 ...
 * @param {Object} resInfo Object containing additional information your respondModule might need to process the response (for example the userID who executed the command). Note: Many core commands expect a steamID: "steamID64" parameter in this object, pointing to the requesting user.
 * @param {String} receiverSteamID steamID64 of the group to receive the comments
 */
function comment(commandHandler, respond, resInfo, receiverSteamID) {
    ...
    syncLoop(acpEntry.amount - (acpEntry.thisIteration + 1), (loop, i) => { // eslint-disable-line no-unused-vars
        setTimeout(() => {
            ...
            var { skipIteration, aSP } = require("../helpers/handleCommentErrors.js").handleCriticalCommentErrors(botindex, "postGroupComment", receiverSteamID, alreadySkippedProxies, acpEntry.amount, res, lang, respond);
            ...
            require("../helpers/getQuote.js").getQuote(acpEntry.quotesArr, lastQuotes, (comment) => { // Get a random quote to comment with and wait for callback to ensure a quote has been found before trying to comment
                ...
                Object.values(commandHandler.controller.bots)[botindex].community.postGroupComment(receiverSteamID, comment, (error) => { // Post comment
                    ...
                    if (error) require("../helpers/handleCommentErrors.js").handleCommentErrors(error, botindex, "postGroupComment", receiverSteamID, acpEntry.amount);
                    ...
                    if (acpEntry.thisIteration == 0) { // Stuff below should only run in first iteration
                        if (commandHandler.data.proxies.length > 1) logger("info", `${logger.colors.fggreen}[${thisbot}] ${acpEntry.amount} Comment(s) requested. Comment in group ${receiverSteamID} with proxy ${Object.values(commandHandler.controller.bots)[botindex].loginData.proxyIndex}: ${String(comment).split("\n")[0]}`);
                        ...
                        if (!error) {
                            if (commandHandler.data.proxies.length > 1) logger("info", `[${thisbot}] Comment ${acpEntry.thisIteration + 1}/${acpEntry.amount} in group ${receiverSteamID} with proxy ${this.loginData.proxyIndex}: ${String(comment).split("\n")[0]}`);
