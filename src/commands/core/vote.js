...
module.exports.upvote = {
    ...
    run: async (commandHandler, args, steamID64, respondModule, context, resInfo) => {
        ...
        let { amountRaw, id } = await getVoteArgs(commandHandler, args, respond);
                ...
                setTimeout(() => {
                    ...
                    bot.community.voteUpSharedfile(sharedfile.id, (error) => {
                        ...
                        if (activeReqEntry.thisIteration == 0) { // Stuff below should only run in first iteration
                            if (commandHandler.data.proxies.length > 1) logger("info", `${logger.colors.fggreen}[${bot.logPrefix}] ${activeReqEntry.amount} Vote(s) requested. Voting on ${id} with proxy ${bot.loginData.proxyIndex}...`);
                                else logger("info", `${logger.colors.fggreen}[${bot.logPrefix}] ${activeReqEntry.amount} Vote(s) requested. Voting on ${id}...`);
                            ...
                            if (!error) {
                                if (commandHandler.data.proxies.length > 1) logger("info", `[${bot.logPrefix}] Voting ${activeReqEntry.thisIteration + 1}/${activeReqEntry.amount} on ${id} with proxy ${bot.loginData.proxyIndex}...`);
                                    else logger("info", `[${bot.logPrefix}] Voting ${activeReqEntry.thisIteration + 1}/${activeReqEntry.amount} on ${id}...`);
                            }
                        }
