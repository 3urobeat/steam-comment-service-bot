...
function attachParentListeners(callback) {
  ...
  handleUncaughtException = (reason) => {
    ...
    if (String(reason).includes("Error: Cannot find module")) {
      ...
      if (global.extdata)
        logger(
          "info",
          "Cannot find module error detected. Trying to fix error by reinstalling modules...\n"
      ...
      require("./controller/helpers/npminteraction.js").reinstallAll(
        logger,
        (err, stdout) => {
          //eslint-disable-line
          if (err) {
            logger(
              "error",
              "I was unable to reinstall all modules. Please try running 'npm install' manually. Error: " +
                err
            );
            process.exit(1);
          } else {
      ...
      logger(
        "error",
        `Uncaught Exception Error! Reason: ${reason.stack}`,
        true
      );
      logger("", "", true);
      logger(
        "warn",
        "Restarting bot in 5 seconds since the application can be in an unrecoverable state..."
      ); // https://nodejs.org/dist/latest-v16.x/docs/api/process.html#process_warning_using_uncaughtexception_correctly
      logger("", "", true);
  ...
  logger.options({
    ...
    paramstructure: [
      logger.Const.TYPE,
      logger.Const.MESSAGE,
      "nodate",
      "remove",
      logger.Const.ANIMATION,
    ],
...
function attachChildListeners() {
  ...
  forkedprocess.on("close", (code) => {
    ...
    logger(
      "warn",
      `Child Process exited with code ${code}! Attempting to restart...`
    );
 ...
 */
module.exports.checkAndGetFile = (
  file,
  logger,
  norequire = false,
  force = false
  ...
  return new Promise((resolve) => {
    ...
    function getNewFile() {
      ...
      let fileurl = `https://raw.githubusercontent.com/HerrEurobeat/steam-comment-service-bot/${branch}/${file.slice(
        2,
        file.length
      )}`; // Remove the dot at the beginning of the file string
      ...
      try {
        ...
        https
          ...
          .on("error", (err) => {
            logger(
              "error",
              `Fatal Error: File '${file}' is corrupted/missing and I can't restore it! Is your internet or GitHub down?\n                     ${err}`,
              true
            );
            resolve(undefined);
          });
        ...
        logger(
          "error",
          `Fatal Error: File ${file} is corrupted/missing and I can't restore it!\n                     ${err}`,
          true
        );
        resolve(undefined);
      }
    ...
    if (!fs.existsSync(file) || force) {
      ...
      if (norequire) {
        ...
        try {
          ...
          if (!file.includes("logger.js") && !file.includes("handleErrors.js"))
            logger(
              "debug",
              `checkAndGetFile(): file ${file} exists, force and norequire are false. Testing integrity by requiring...`
            ); // Ignore message for logger.js as it won't use the real logger yet
          ...
          logger(
            "warn",
            `It looks like file ${file} is corrupted. Trying to pull new file from GitHub...`,
            false,
            true
          );
...
module.exports.run = () => {
  ...
  attachParentListeners(async () => {
    ...
    // get file to start
    let file = await this.checkAndGetFile(
      "./src/controller/controller.js",
      logger,
      false,
      false
    ); // We can call without norequire as process.argv[3] is set to 0 (see top of this file) to check controller.js for errors as well
    if (!file) return;

    forkedprocess = cp.fork("./src/controller/controller.js", [
      __dirname,
      Date.now(),
    ]); // Create new process and provide srcdir and timestamp as argv parameters
  ...
  attachParentListeners(() => {
    ...
    setTimeout(async () => {
      ...
      forkedprocess = cp.fork("./src/controller/controller.js", [
        ...
        Date.now(),
        JSON.stringify(args),
      ]); // Create new process and provide srcdir, timestamp and restartargs as argv parameters
      childpid = forkedprocess.pid;
