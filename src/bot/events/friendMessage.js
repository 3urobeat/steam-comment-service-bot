 ...
 * Author: 3urobeat
 *
 * Last Modified: 27.03.2023 13:57:25
...
Bot.prototype._attachSteamFriendMessageEvent = function() {
    ...
    this.user.on("friendMessage", (steamID, message) => {
        ...
        function chatmsg(steamID, txt, retry) { // Sadly needed to be included here in order to access bot instance before friendMessage got called at least once or needing to provide it as parameter
            ...
            if (txt.length > 1000) {
                logger("warn", `[${_this.logPrefix}] The bot tried to send a chat message that's longer than 1000 chars. Cutting it to 996 chars to reduce the risk of a RateLimitExceeded error!`);
                txt = txt.slice(0, 996);
                txt += "...";
            }
        ...
        if (this.index === 0) { // Check if this is the main bot

            /**
             * Function to quickly respond with owneronly message and stop command execution
             */
            var notownerresponse = (() => {
                return chatmsg(steamID, this.controller.data.lang.commandowneronly);
            });
            ...
            switch(cont[0].toLowerCase()) {
                ...
                case "!ping":
                    require("../commands/general.js").ping(chatmsg, steamID, this.controller.data.lang);
                    break;
            ...
            switch(message.toLowerCase()) {
                case "!about": // Please don't change this message as it gives credit to me; the person who put really much of his free time into this project. The bot will still refer to you - the operator of this instance.
                    chatmsg(steamID, this.controller.data.datafile.aboutstr);
                    break;
                default:
                    if (message.startsWith("!")) chatmsg(steamID, `${this.controller.data.lang.childbotmessage}\nhttps://steamcommunity.com/profiles/${new SteamID(String(this.controller.main.steamID)).getSteamID64()}`);
                        else logger("debug", "Chat message is not a command, ignoring message.");
            }
        }
