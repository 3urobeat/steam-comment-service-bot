 ...
 * Author: 3urobeat
 * 
 * Last Modified: 09.03.2022 13:25:19
...
module.exports.run = (str, profileIDType, callback) => {
    ...
    function handleResponse(err, res) { //eslint-disable-line
        ...
        if (err) {
            ...
            if (new SteamID(res).type != profileIDType) callback(`Recieved steamID of type ${new SteamID(res).type} but expected ${profileIDType}.`, null); 
    ...
    if (isNaN(str) || !new SteamID(str).isValid()) {
        if (str.includes("steamcommunity.com/id/")) {
            ...
            if (profileIDType == SteamID.Type.INDIVIDUAL) {
                logger("debug", "getCommentArgs(): User didn't provide a full url as profileID str. Expecting custom profile URL based on profileIDType...")

                steamIDResolver.customUrlTosteamID64(str, handleResponse)

            } else if (profileIDType == SteamID.Type.CLAN) {
                logger("debug", "getCommentArgs(): User didn't provide a full url as profileID str. Expecting custom group URL based on profileIDType...")

                steamIDResolver.groupUrlToGroupID64(str, handleResponse)
                
            } else {
                logger("debug", "getCommentArgs(): Sending error message and aborting as user provided something as profileID argument which I don't understand: " + str);
        ...
        if (new SteamID(str).type != profileIDType) handleResponse(`Recieved steamID of type ${new SteamID(str).type} but expected ${profileIDType}.`, null); 
