 ...
 */
module.exports.retryComments = (receiverSteamID, steamID, lang, res, respond, callback) => {
    ...
    if (advancedconfig.retryFailedComments && acpEntry.retryAttempt < advancedconfig.retryFailedCommentsAttempts && Object.keys(mainfile.failedcomments[receiverSteamID]).length > 0) {
        ...
        logger("info", `${Object.keys(mainfile.failedcomments[receiverSteamID]).length} comments failed for ${receiverSteamID}. Retrying in ${round(advancedconfig.retryFailedCommentsDelay / 60000, 2)} minutes (Attempt ${acpEntry.retryAttempt}/${advancedconfig.retryFailedCommentsAttempts})`);
        respond(202, lang.commentretrying.replace("failedamount", Object.keys(mainfile.failedcomments[receiverSteamID]).length).replace("numberOfComments", acpEntry.amount).replace("retrydelay", round(advancedconfig.retryFailedCommentsDelay / 60000, 2)).replace("thisattempt", acpEntry.retryAttempt).replace("maxattempt", advancedconfig.retryFailedCommentsAttempts));

        // Wait retryFailedCommentsDelay ms before retrying failed comments
        setTimeout(() => {
            // Increase numberOfComments by amount of failed comments
            acpEntry.amount = Number(acpEntry.amount) + Object.keys(mainfile.failedcomments[receiverSteamID]).length;

            // Increase until value (amount of retried comments * commentdelay) + delay before starting retry attempts
            acpEntry.until = Number(acpEntry.until) + (Object.keys(mainfile.failedcomments[receiverSteamID]).length * config.commentdelay) + advancedconfig.retryFailedCommentsDelay;

            // Reset content in failedcomments obj
            mainfile.failedcomments[receiverSteamID] = {};

            // Call comment function again
            if (acpEntry.type == "profile") require("../commands/commentprofile.js").comment(receiverSteamID, steamID, lang, res, respond);
                else require("../commands/commentgroup.js").comment(receiverSteamID, steamID, lang, res, respond);
