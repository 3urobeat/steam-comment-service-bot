 ...
 * Author: 3urobeat
 *
 * Last Modified: 16.04.2023 19:12:12
...
Bot.prototype.sendChatMessage = function(_this, resInfo, txt, retry, part = 0) {
    ...
    if (_this.controller.data.advancedconfig.printDebug) {
        logger("debug", `[${_this.logPrefix}] Sending message (${txt.length} chars) to ${resInfo.steamID64} (retry: ${retry == true}, part: ${part}): "${thisPart.replace(/\n/g, "\\n")}"`); // Intentionally checking for == true to prevent showing undefined
    } else {
        if (txt.length >= 75) logger("info", `[${_this.logPrefix}] Sending message to ${resInfo.steamID64}: "${thisPart.slice(0, 75).replace(/\n/g, "\\n") + "..."}"`);
    ...
    _this.user.chat.sendFriendMessage(resInfo.steamID64, txt, {}, (err) => {
        if (err) { // Check for error as some chat messages seem to not get send lately
            logger("warn", `[${_this.logPrefix}] Error trying to send chat message of length ${txt.length} to ${resInfo.steamID64}! ${err}`);
            ...
            setTimeout(() => {
                this.sendChatMessage(_this, resInfo, txt, retry, part); // Send the same part again
            }, 10000);
            ...
            if (750 * (part + 1) <= txt.length) {
                logger("info", `Message longer than 750 chars, sending next part to '${resInfo.steamID64}' in 7.5 seconds...`, false, false, logger.animation("waiting"));
                setTimeout(() => this.sendChatMessage(_this, resInfo, txt, retry, part + 1), 7500);
            } else {
