 ...
 * Author: 3urobeat
 * 
 * Last Modified: 12.03.2022 11:31:56
...
module.exports.run = async (chatmsg, steamID, args, lang, res, lastcommentdoc) => {
    ...
    mainfile.activecommentprocess[recieverSteamID]["interval"] = setInterval(() => {
        setTimeout(() => { //timeout for retryFailedComments
            ...
            require("../helpers/getQuote.js").getQuote(quotesArr, lastQuotes, (comment) => { //get a random quote to comment with and wait for callback to ensure a quote has been found before trying to comment
                ...
                controller.communityobject[thisindex].postGroupComment(steamID, comment, (error) => { //post comment
                    ...
                    if (error) {
                        if (require("../helpers/handleCommentErrors.js").handleCommentErrors(error, thisindex, thisIterationLocal, "postGroupComment", recieverSteamID, numberOfComments)) return;
                    }
                    ...
                    if (thisIterationLocal == 0) { //Stuff below should only run in first iteration (main bot)
                        ...
                        if (!error) {
                            if (loginfile.proxies.length > 1) logger("info", `[${thisbot}] Comment ${thisIterationLocal + 1}/${numberOfComments} in group ${recieverSteamID} with proxy ${loginfile.additionalaccinfo[thisindex].thisproxyindex}: ${String(comment).split("\n")[0]}`)
                                else logger("info", `[${thisbot}] Comment ${thisIterationLocal + 1}/${numberOfComments} in group ${recieverSteamID}: ${String(comment).split("\n")[0]}`) //splitting \n to only get first line of multi line comments
                        }
                    }
