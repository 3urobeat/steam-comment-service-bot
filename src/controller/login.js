 ...
 * Author: 3urobeat
 *
 * Last Modified: 2025-07-17 12:10:05
    ...
    const allAccsOnlineInterval = setInterval(() => {
        ...
        if (allAccountsNotPopulated.length > 0) {
            ...
            if (lastWaitingForMsgAmount != allAccountsNotPopulated.length) {
                logger("info", `All accounts logged in, waiting for Steam to provide full information of bot(s) '${allAccountsNotPopulated.flatMap((e) => e.index).join(", ")}'...`, false, true, logger.animation("waiting"));
